...
class Balance(TimeStampedModel):
    ...
    def __str__(self):
        if self.balance_amount > 0:
            ...
            return f"{self.user1.username} owes ₹{abs(self.balance_amount)} to {self.user2.username}"
        else:
...
class UserTotalBalance(models.Model):
    ...
    def __str__(self):
        if self.total_balance > 0:
            ...
            return f"{self.user1.username} owes ₹{abs(self.total_balance)} to {self.user2.username}"
        else:
            return f"{self.user1.username} and {self.user2.username} are settled"
